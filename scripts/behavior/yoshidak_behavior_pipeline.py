...
ROOT = PROJECT_ROOT
DATA_ROOT = YOSHIDAK_BEHAVIOR_ROOT
OUTDIR = OUT_V6_YOSHIDAK
OLD_OUTDIR_15FPS = OUT_V6_OLD_15FPS
OLD_OUTDIR = OUT_V6
...
def parse_subject_id(path: str) -> str:
    ...
    m = re.search(r"(female\d+|male\d+)", b)
    if not m:
        raise ValueError(f"Could not parse subject id from filename: {path}")
    return m.group(1)


def parse_time_of_day(path: str) -> str:
    b = os.path.basename(path).lower()
    m = re.search(r"-(am|pm)_", b)
    if not m:
        raise ValueError(f"Could not parse am/pm from filename: {path}")
    return m.group(1)


def parse_camera(path: str) -> str:
    b = os.path.basename(path).lower()
    m = re.match(r"(cam\d+)-", b)
    if not m:
        raise ValueError(f"Could not parse camera from filename: {path}")
    return m.group(1)
...
def summarize_subject_age(per_video: pd.DataFrame) -> pd.DataFrame:
    ...
    out = (
        ...
        .agg(agg)
        .rename(columns={"video_path": "n_videos", "am_pm": "n_am_videos", "camera": "cameras"})
        .sort_values(["sex", "subject", "age_months"])
        .reset_index(drop=True)
    )
    out["n_pm_videos"] = out["n_videos"] - out["n_am_videos"]
    return out


def resolve_old_metrics_csv() -> Tuple[Path | None, str]:
    candidates = [
        (OLD_OUTDIR_15FPS / "per_video_metrics_robust.csv", "old_out_v6_15fps"),
        (OLD_OUTDIR / "per_video_metrics_robust.csv", "old_out_v6"),
    ]
...
def load_old_subject_age_summary() -> Tuple[pd.DataFrame, str]:
    old_csv, old_label = resolve_old_metrics_csv()
    if old_csv is None:
        return pd.DataFrame(), old_label
    if not old_csv.exists():
        return pd.DataFrame(), old_label

    old = pd.read_csv(old_csv)
    needed = {"sex", "subject", "age_months"}
    if not needed.issubset(old.columns):
        return pd.DataFrame(), old_label

    old_agg = {"video_path": "count"} if "video_path" in old.columns else {}
    summary_cols = [
        "fps",
        "duration_s",
        "frac_missing_head",
    ...
    for col in [c for c in summary_cols if c in old.columns] + [m for m in METRICS if m in old.columns]:
        old_agg[col] = "median"

    old_subject_age = (
        old.groupby(["sex", "subject", "age_months"], as_index=False)
        .agg(old_agg)
        .rename(columns={"video_path": "n_videos"})
        .sort_values(["sex", "subject", "age_months"])
        .reset_index(drop=True)
    )
    return old_subject_age, old_label


def compare_to_old_out_v6(new_subject_age: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, str]:
    old_subject_age, old_label = load_old_subject_age_summary()
    if old_subject_age.empty:
        return pd.DataFrame(), pd.DataFrame(), old_label

    shared_metrics = [m for m in METRICS if m in old_subject_age.columns and m in new_subject_age.columns]
    group_keys = ["sex", "age_months"]

    def grouped(df: pd.DataFrame, label: str) -> pd.DataFrame:
        agg = {m: "median" for m in shared_metrics}
        agg["subject"] = "nunique"
        out = df.groupby(group_keys, as_index=False).agg(agg).rename(columns={"subject": "n_subjects"})
        out["dataset"] = label
        return out

    new_group = grouped(new_subject_age, "yoshidak")
    old_group = grouped(old_subject_age, old_label)
    merged = new_group.merge(old_group, on=group_keys, suffixes=("_new", "_old"), how="outer")

    rows = []
    for _, row in merged.iterrows():
        base = {"sex": row["sex"], "age_months": row["age_months"]}
        for m in shared_metrics:
            new_val = row.get(f"{m}_new", np.nan)
            old_val = row.get(f"{m}_old", np.nan)
            rows.append(
                {
                    **base,
                    "metric": m,
    ...
    return old_subject_age, pd.DataFrame(rows), old_label


def build_pooled_overlap_subject_age(new_subject_age: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, str]:
    old_subject_age, old_label = load_old_subject_age_summary()
    if old_subject_age.empty:
        return pd.DataFrame(), pd.DataFrame(), old_label

    overlap_ages = sorted(set(new_subject_age["age_months"].tolist()) & set(old_subject_age["age_months"].tolist()))
    if not overlap_ages:
        return pd.DataFrame(), pd.DataFrame(), old_label

    new_overlap = new_subject_age.loc[new_subject_age["age_months"].isin(overlap_ages)].copy()
    old_overlap = old_subject_age.loc[old_subject_age["age_months"].isin(overlap_ages)].copy()
    new_overlap["dataset"] = "yoshidak"
    old_overlap["dataset"] = old_label
    new_overlap["tank_small"] = 1
    old_overlap["tank_small"] = 0
    pooled = pd.concat([new_overlap, old_overlap], ignore_index=True, sort=False)
    pooled["group_id"] = pooled["dataset"].astype(str) + ":" + pooled["subject"].astype(str)
    ...
    summary_cols = [
        ...
        "frame_width_px",
        "frame_height_px",
    ] + [m for m in METRICS if m in pooled.columns]
    group_agg = {"subject": "nunique"}
    for col in summary_cols:
        group_agg[col] = "median"
    grouped = (
        pooled.groupby(["dataset", "tank_small", "sex", "age_months"], as_index=False)
        .agg(group_agg)
        .rename(columns={"subject": "n_subjects"})
        .sort_values(["tank_small", "sex", "age_months", "dataset"])
        .reset_index(drop=True)
    )
    return pooled, grouped, old_label


def fit_pooled_tank_models(pooled_subject_age: pd.DataFrame) -> pd.DataFrame:
    if pooled_subject_age.empty:
        return pd.DataFrame()
    if smf is None:
    ...
    for metric in METRICS:
        if metric not in pooled_subject_age.columns:
            continue
        d = pooled_subject_age[["group_id", "sex", "age_months", "tank_small", metric]].dropna().copy()
        if d["tank_small"].nunique() < 2:
            rows.append({"metric": metric, "note": "missing_in_old_or_new_dataset"})
            continue
        if d["group_id"].nunique() < 6 or d["sex"].nunique() < 2:
        ...
        try:
            res = smf.mixedlm("y ~ tank_small + age_c + sex + tank_small:sex", d, groups=d["group_id"]).fit(reml=False)
            ...
            rows.append(
                {
                    "metric": metric,
                    "n_rows": len(d),
                    "n_subjects": d["group_id"].nunique(),
                    "ages": ",".join(map(str, sorted(d["age_months"].unique().tolist()))),
                    "coef_tank_small": params.get("tank_small", np.nan),
                    "p_tank_small": pvals.get("tank_small", np.nan),
                    ...
                    "coef_tank_by_sex_male": params.get("tank_small:sex[T.male]", np.nan),
                    "p_tank_by_sex_male": pvals.get("tank_small:sex[T.male]", np.nan),
    ...
    if "p_tank_small" in out.columns:
        out["q_tank_small"] = bh_qvalues(out["p_tank_small"])
    if "p_tank_by_sex_male" in out.columns:
        out["q_tank_by_sex_male"] = bh_qvalues(out["p_tank_by_sex_male"])
    return out
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run robust behavior metrics for the YOSHIDAK killifish videos."
    )
    parser.add_argument("--data-root", default=str(DATA_ROOT), help="Folder to scan for YOSHIDAK .avi files.")
    parser.add_argument("--outdir", default=str(OUTDIR), help="Output directory for YOSHIDAK v6 results.")
    parser.add_argument(
        "--old-outdir-15fps",
        default=str(OLD_OUTDIR_15FPS),
        help="Comparison output directory from the old 15 fps pipeline.",
    )
    parser.add_argument(
        "--old-outdir",
        default=str(OLD_OUTDIR),
        help="Comparison output directory from the original v6 pipeline.",
    )
    return parser.parse_args()


def main() -> None:
    global DATA_ROOT, OUTDIR, OLD_OUTDIR_15FPS, OLD_OUTDIR

    args = parse_args()
    DATA_ROOT = Path(args.data_root)
    OUTDIR = Path(args.outdir)
    OLD_OUTDIR_15FPS = Path(args.old_outdir_15fps)
    OLD_OUTDIR = Path(args.old_outdir)
    ...
    for vp in videos:
        try:
            ...
            am_pm = parse_time_of_day(vp)
            camera = parse_camera(vp)
    ...
    per_video = pd.DataFrame(rows).sort_values(["sex", "subject", "age_months", "am_pm", "camera"])
    ...
    overview = pd.DataFrame(
        {
            ...
            "cameras": [",".join(sorted(per_video["camera"].dropna().unique().tolist()))],
        }
    )
    ...
    old_subject_age, comparison, old_label = compare_to_old_out_v6(subject_age)
    if not old_subject_age.empty:
        old_subject_age.to_csv(OUTDIR / "old_out_v6_subject_age_summary.csv", index=False)
    if not comparison.empty:
        comparison.to_csv(OUTDIR / "comparison_to_out_v6_by_age_sex.csv", index=False)

    pooled_overlap, pooled_overlap_summary, old_label = build_pooled_overlap_subject_age(subject_age)
    pooled_tank = fit_pooled_tank_models(pooled_overlap)
    if not pooled_overlap.empty:
        pooled_overlap.to_csv(OUTDIR / "pooled_overlap_subject_age.csv", index=False)
        pooled_overlap_summary.to_csv(OUTDIR / "pooled_overlap_group_summary.csv", index=False)
    if not pooled_tank.empty:
        pooled_tank.to_csv(OUTDIR / "pooled_tank_effects_mixedlm.csv", index=False)
        if "p_tank_small" in pooled_tank.columns:
            pooled_tank.sort_values(["q_tank_small", "p_tank_small", "metric"]).to_csv(
                OUTDIR / "pooled_tank_effects_ranked.csv",
                index=False,
            )
    ...
    if not comparison.empty:
        print(f"Saved old-vs-new comparison: {OUTDIR / 'comparison_to_out_v6_by_age_sex.csv'}")
        print(f"Comparison source: {old_label}")
    if not pooled_overlap.empty:
        print(f"Saved pooled overlap table: {OUTDIR / 'pooled_overlap_subject_age.csv'}")
        print(f"Saved pooled overlap summary: {OUTDIR / 'pooled_overlap_group_summary.csv'}")
    if not pooled_tank.empty:
        print(f"Saved pooled tank mixed models: {OUTDIR / 'pooled_tank_effects_mixedlm.csv'}")
    if len(err_df):
